...
const filterWork = (function() {
    ...
    tags.forEach(tag =>
        tag.addEventListener('click', () => {
            let newList = workListArray.filter(entry =>
                checkForTags(entry, tag.innerHTML)
            );
...
const searchPosts = (function() {
    ...
    if (rawSearchCriteria) {
        ...
        if (!searchCount) {
            document.querySelector('.blog__search-title').style.marginTop =
                '100px';
